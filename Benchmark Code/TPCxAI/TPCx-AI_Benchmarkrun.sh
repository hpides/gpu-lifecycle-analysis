...
./bin/tpcxai.sh --phase {CLEAN,DATA_GENERATION,LOADING,TRAINING,SERVING,SERVING,SERVING_THROUGHPUT,SCORING,CHECK_INTEGRITY} -uc {2,5,9} -sf ${TPCxAI_SCALE_FACTOR}  --streams ${TPCxAI_SERVING_THROUGHPUT_STREAMS}  -c ${TPCxAI_CONFIG_FILE_PATH} ${VFLAG}
...
ole.becker@lx01:~/benchmarks/tpcxai/tpc$ ^C
ole.becker@lx01:~/benchmarks/tpcxai/tpc$ cat TPCx-AI_Benchmarkrun.sh
#!/bin/bash
